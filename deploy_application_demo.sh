...
PRIMARY_SECRET_OCID="ocid1.vaultsecret.oc1.eu-frankfurt-1.xxxxxxx"
PG_USER="phantompete"
PG_DB="postgres"
PG_HOST="your_postgresql_host"

#Go Cache Configuration
CACHE_HOST="your_valkey_host:6379"
...
unzip -o "$GO_API_ZIP" -d Livelab_WebApp
cd Livelab_WebApp || exit 1
cd backend || exit 1
...
sed -i "s|var redis_host = \".*\"|var redis_host = \"$CACHE_HOST\"|" main.go
sed -i "s|var api_key = \".*\"|var api_key = \"$TRANSPORT_API_KEY\"|" main.go
...
sudo semanage fcontext -a -t bin_t "$WEBAPP_HOME/Livelab_WebApp/backend/go-livelab-webapp"
sudo restorecon -v $WEBAPP_HOME/Livelab_WebApp/backend/go-livelab-webapp
...
sed -i "s|^WorkingDirectory=.*|WorkingDirectory=${WEBAPP_HOME}/Livelab_WebApp/frontend|" "frontend.service"
sed -i "s|^WorkingDirectory=.*|WorkingDirectory=${WEBAPP_HOME}/Livelab_WebApp/backend|" "backend.service"
sed -i "s|^ExecStart=.*|ExecStart=${WEBAPP_HOME}/Livelab_WebApp/backend/go-livelab-webapp|" "backend.service"
